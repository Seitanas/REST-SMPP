...
def TokenGen(json_data):
    ...
    db = QueryDB()
    reply = db.execute("SELECT id, password FROM users "
                       "WHERE username=\"%s\"" %
                       json_data["auth"]["username"])
    if len(reply):
        cfg = Config.ReadConfig()
        salt = cfg.config.get('security', 'salt')
        md5hash = hashlib.md5(
                    str(salt).encode('utf-8') +
                    str(json_data["auth"]["password"]).
        ...
        for pw in reply:
            if pw[1] != md5hash:
                ...
                db.commit('INSERT INTO '
                          'tokens(userid, token, created, expires)'
                          'VALUES(\"%s\", \"%s\", \"%s\", \"%s\")' %
                          (pw[0], token, created, expires))
                reply = {}
                reply['token'] = {}
...
class AuthResource:
    ...
    def on_post(self, req, resp):
        ...
        logger = logging.getLogger('REST-SMPP')
        logger.debug("Got HTTP AUTH request from: %s", req.access_route[0])
        try:
        ...
        if "auth" in json_data:
            if ("username" in json_data["auth"] and
                    "password" in json_data["auth"]):
